...
const Registration = () => {
  ...
  if (debugging) {
    ui = (
      <View>
        <Text>currentValue={JSON.stringify(currentValue, null, 5)}</Text>
      </View>
    );
  }
  ...
  return (
    <View>
      <Text style={{fontSize: 24}}>TeamUp sign in/sign up</Text>
      <View style={{padding: 10, margin: 10, backgroundColor: '#ddd'}}>
        ...
        {currentValue.email == '' ? (
          <TouchableOpacity onPress={() => registerEmail(email)}>
